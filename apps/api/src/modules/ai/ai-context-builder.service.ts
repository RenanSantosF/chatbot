 ...
 * quer que ela se comporte, as instruções que o dono ensinou, e o histórico
 * recente da conversa atual. Isso é o que impede a IA de inventar política
 * da empresa — se não está no prompt, ela não sabe.
 */
@Injectable()
export class AiContextBuilder {
  ...
  private buildSystemPrompt(params: {
    ...
    const lines = [
      ...
      'Nunca invente informação sobre a empresa (preço, prazo, política) que não esteja nas instruções abaixo — se não souber, diga que vai verificar com a equipe.',
  ...
  private async buildIdentityPrompt(): Promise<string> {
  ...
  async buildStandalone(message: string): Promise<AiConversationContext> {
    const systemPrompt = await this.buildIdentityPrompt();
